"""Drop the two split-vote picks (mid 3084136, 3084135) and look for replacements among
the remaining not-yet-started candidates, this time requiring full agreement (every
contributing book on the same side of 0.5, no ties) on top of the existing fixed rule.
Does not change the fixed rule itself -- this is an extra acceptance filter for which
of the rule's predictions get shown as "recommended", applied going forward only.
"""
...
locked_out = [p for p in totals.read_json(OUT / 'locked_predictions.json') if p['id'] not in DROP_IDS]
print(json.dumps({'candidates_total': len(candidates), 'already_scanned': len(attempts),
                   'kept_after_drop': [p['id'] for p in locked_out]}, ensure_ascii=False), flush=True)
...
for i, m in enumerate(candidates[RESUME_FROM:], RESUME_FROM + 1):
    ...
    unanimous = False
    if entry.get('side'):
        group = [q for q in entry['quotes'] if q['line'] == entry['line']]
        inds = [(1/(1+q['over']))/(1/(1+q['over'])+1/(1+q['under'])) for q in group]
        sides = ['over' if v > 0.5 else 'under' if v < 0.5 else 'tie' for v in inds]
        unanimous = len(set(sides)) == 1 and 'tie' not in sides

    print(json.dumps({'i': i, 'mid': mid, 'match': f"{m['home']} vs {m['away']}", 'kickoff': m['kickoff'],
                       'side': entry.get('side'), 'unanimous': unanimous, 'reason': entry.get('reason')},
                      ensure_ascii=False), flush=True)

    if entry.get('side') and unanimous:
        new_locked.append({'id': entry['id'], 'league': entry['league'], 'home': entry['home'], 'away': entry['away'],
                            ...
                            'record_time': entry['record_time'], 'unanimous': True, 'locked_at': core.stamp()})
    if len(new_locked) >= TARGET_NEW or core.SOURCE_LIMITED:
        break

locked_out.extend(new_locked)
...
print(json.dumps({'final_locked_count': len(locked_out), 'newly_added': len(new_locked),
                   'scanned_total': len(attempts), 'source_limited': core.SOURCE_LIMITED}, ensure_ascii=False), flush=True)
